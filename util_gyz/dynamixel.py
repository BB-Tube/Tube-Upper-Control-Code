...
### class Dynamixel(Motor, PacketManager, MotorTable):
class Dynamixel(Motor): 
    ...
    def _read_address(self, address_enum, signed = False): 
        ...
        # convert
        read *= self.mv.get_conversion(self.model, address_enum)
        unit_internal = self.mv.get_unit(address_enum)      
        converted_to_external = self.units.convert(read, unit_internal)
        # print("Read : ", unit_read, " Unit Out ", unit_out)
        # print("Converted : ", convert(read, unit_read, unit_out))
        return converted_to_external #convert(read, unit_read, unit_out)
    ...
    def _write_address(self, address_enum, data):
        ...
        #conversion stuffs
        unit_internal = self.mv.get_unit(address_enum)
        unit_external = self.units._get_external_unit_used(unit_internal)
        data_in_internal_unit = convert_units(data, unit_external, unit_internal)
        data_in_tick = int(data_in_internal_unit / self.mv.get_conversion(self.model, address_enum))
    ...
    def get_mode(self):
        return self.mv.Mode(self._read_address(self.mv.Address.OPERATING_MODE))
 
    def set_mode(self, mode):
        self._write_address(self.mv.Address.OPERATING_MODE, mode.value)
    ...
    def set_protoccol_type(self, value):
    ...
    def set_homing_offset(self, value):
    ...
    def get_startup_configuration(self, value):
    ...
    def get_enable(self, name = False):
        enable = bool(self._read_address(self.mv.Address.TORQUE_ENABLE))
        if name:
            return [self.name, enable]
        else:
            return enable
